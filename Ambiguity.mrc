...
on ^*:NOTICE:*:#:{
    haltdef
    if (*invited*into the channel* iswm $1-) {
        echo $color(notice) -t $chan $c2(⁞) $+($c1(«),$c2(«) $b($c2($chr(40))),$nick,$b(/),$chan,$b($c2($chr(41)))) ⁞ $b($1) invited $b($3) into the channel.
    }
    else {
...
on ^*:NOTICE:*:?:{
    haltdef
    if ($cid != $activecid) {
        scid $v1 echo -cst notice $+(,$c1(«),$c2(«),$chr(32),$c2($chr(40)),,$nick,,$c2($chr(41)),,:) $replace($1-,$chr(124),$!chr(124),$chr(36),$!chr(36),$chr(37),$!chr(37))
    }
    else {
        echo -catm notice $+(,$c1(«),$c2(«),$chr(32),$c2($chr(40)),,$nick,,$c2($chr(41)),,:) $1-
    }
...
on *:DNS:{
    ...
    var %d $dns(0)
    if (%d == 0) {
        echo -a $pre DNS: Unable to resolve host
    }
    else {
        if (%d > 1) {
            echo -a $c2(-----<dns>------------------------------)
...
alias msg {
    ...
    .raw privmsg $$1 $$2-
    if ($left($1,1) isin +%@&~) {
        var %c $remove($1,+,%,@,&,~) | if ($me ison %c) { echo -ct msg %c $c2(⁞) $c2(「) $b($regsubex($pnick(%c,$me),/^([~&@%+])?(.+)/,$iif(\2,\1) $+ $+(,$nick(%c,$me).color,$me,))) $+($chr(40),$ccolor($left($1,1)),$chr(41)) $c2(」) $2- }
    }
    else {
        var %c $1 | $iif($window($1),echo -ct msg $1 $c2(⁞) 「 $b($iif($me ison %c,$regsubex($pnick(%c,$me),/^([~&@%+])?(.+)/,$iif(\2,\1) $+ $+(,$nick(%c,$me).color,$me,)),$+ $mc($me))) 」 $2-)
    }
